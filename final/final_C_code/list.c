...
// NOTE: calloc is used to prevent errrors when freeing data that doesn't exists, this happened due to the optimizations and occure in the deleteList functions
//       but then again, I'm not sure about this. No time was left for debugging
...
void addList(List *list, void *data)
{
    ...
    list->add_node->data = calloc(1,list->data_size);
    memcpy(list->add_node->data, data, list->data_size);
    list->add_node->next = calloc(1,sizeof(Node));
...
{
    void **array = calloc(1,list->length * sizeof(void*));
